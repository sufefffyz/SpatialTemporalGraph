...
def _block_random_default_ratios(total_ratio: float) -> tuple[float, float]:
    if abs(total_ratio - 0.3) < 1e-8:
        return 0.6, 0.5
    if abs(total_ratio - 0.1) < 1e-8:
        return 0.5, 0.2

    window_ratio = min(1.0, max(total_ratio, total_ratio ** 0.5))
    node_ratio = total_ratio / window_ratio
    return window_ratio, node_ratio


def _block_random_window_node_ratios(total_ratio: float) -> tuple[float, float]:
    window_env = os.environ.get("DYNAMIC_PRUNING_WINDOW_RATIO", "").strip()
    node_env = os.environ.get("DYNAMIC_PRUNING_NODE_RATIO", "").strip()

    if window_env and node_env:
        window_ratio = float(window_env)
        node_ratio = float(node_env)
    elif window_env:
        window_ratio = float(window_env)
        node_ratio = total_ratio / window_ratio
    elif node_env:
        node_ratio = float(node_env)
        window_ratio = total_ratio / node_ratio
    else:
        window_ratio, node_ratio = _block_random_default_ratios(total_ratio)

    window_ratio = _validate_ratio("DYNAMIC_PRUNING_WINDOW_RATIO", window_ratio)
    node_ratio = _validate_ratio("DYNAMIC_PRUNING_NODE_RATIO", node_ratio)
    effective_ratio = window_ratio * node_ratio
    if abs(effective_ratio - total_ratio) > 1e-6:
        raise ValueError(
            "block_random_node_window ratios must multiply to DYNAMIC_PRUNING_RATIO, "
            f"got window_ratio={window_ratio}, node_ratio={node_ratio}, "
            f"effective={effective_ratio}, target={total_ratio}."
        )
    return window_ratio, node_ratio
...
def apply_dynamic_pruning_cfg(cfg: EasyDict, backbone_tag: str, strategy: str) -> EasyDict:
    ...
    if strategy in {"block_random", "random_node_window"}:
        strategy = "block_random_node_window"
    cfg = copy.deepcopy(cfg)

    ratio = float(os.environ.get("DYNAMIC_PRUNING_RATIO", "0.1"))
    target_effective_node_window_ratio = None
    if strategy == "block_random_node_window":
        window_ratio, block_node_retention_ratio = _block_random_window_node_ratios(ratio)
        target_effective_node_window_ratio = window_ratio * block_node_retention_ratio
        ratio = window_ratio
    ...
    node_retention_ratio = (
        float(block_node_retention_ratio)
        if strategy == "block_random_node_window"
        else float(os.environ.get("DYNAMIC_PRUNING_NODE_RATIO", str(ratio)))
    )
    ...
    cfg.TRAIN.DYNAMIC_PRUNING = EasyDict(
        {
            ...
            "TARGET_EFFECTIVE_NODE_WINDOW_RATIO": target_effective_node_window_ratio,
    ...
    elif strategy == "block_random_node_window":
        _append_ckpt_tag(
            cfg,
            f"dynprune_{backbone_tag}_{strategy_tag}_wr{_ratio_tag(ratio)}_nr{_ratio_tag(node_retention_ratio)}"
            f"_eff{_ratio_tag(target_effective_node_window_ratio)}{hparam_tag}_s{seed}",
        )
        cfg.DESCRIPTION = (
            f"{cfg.DESCRIPTION} [dynamic_pruning:{strategy_tag}, effective_strategy={strategy}, "
            f"window_ratio={ratio}, node_retention_ratio={node_retention_ratio}, "
            f"target_effective_node_window_ratio={target_effective_node_window_ratio}, "
