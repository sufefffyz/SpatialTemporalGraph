...
raw_adj = np.eye(int(DESC["num_nodes"]), dtype=np.float32)
...
CFG = EasyDict()
CFG.DESCRIPTION = "GraphWaveNet on SD with identity adjacency"
CFG.GPU_NUM = 1
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    "checkpoints",
    MODEL_ARCH.__name__,
    "_".join([DATA_NAME, "identity", str(CFG.TRAIN.NUM_EPOCHS), str(INPUT_LEN), str(OUTPUT_LEN)]),
)
