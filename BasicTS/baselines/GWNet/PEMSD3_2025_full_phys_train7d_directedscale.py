...
adj_mx, _ = load_adj('datasets/' + DATA_NAME + '/adj_mx.pkl', 'transition')
...
CFG = EasyDict()
CFG.DESCRIPTION = 'PeMS MVP pilot on ' + DATA_NAME + ' (train7d_directedscale)'
CFG.GPU_NUM = 1
...
CFG.WANDB.RUN_NAME = f'{MODEL_ARCH.__name__}_{DATA_NAME}_train7d'
CFG.WANDB.GROUP = DATA_NAME
CFG.WANDB.TAGS = ['pems-mvp', 'train7d']
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    MODEL_ARCH.__name__,
    '_'.join([DATA_NAME, 'train7d_directedscale', str(CFG.TRAIN.NUM_EPOCHS), str(INPUT_LEN), str(OUTPUT_LEN)])
)
