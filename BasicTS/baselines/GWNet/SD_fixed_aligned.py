...
CFG = EasyDict()
CFG.DESCRIPTION = "GraphWaveNet on LargeST SD original fixed graph with aligned dynamic-threshold settings"
CFG.GPU_NUM = 1
...
ckpt_name_parts = [DATA_NAME, "original_fixed_aligned", str(CFG.TRAIN.NUM_EPOCHS), str(INPUT_LEN), str(OUTPUT_LEN)]
if RUN_TAG:
...
CFG.WANDB.RUN_NAME = os.environ.get(
    "WANDB_NAME",
    f"GraphWaveNet_{DATA_NAME}_original_fixed_aligned_{RUN_TAG}".rstrip("_"),
)
CFG.WANDB.GROUP = os.environ.get("WANDB_RUN_GROUP", "sd_original_fixed_aligned_gwnet")
CFG.WANDB.TAGS = ["adaptive-threshold", "sd", "original-fixed", "gwnet", "aligned-baseline"]
