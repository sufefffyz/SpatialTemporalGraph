...
def embed_sensors_into_graph(G, edges_gdf, nodes_gdf, sensors):
    ...
    for link_id, s_list in edge_sensors.items():
        ...
        has_geom = "geometry" in original_data and original_data["geometry"] is not None
        ...
        for i in range(len(chain) - 1):
            ...
            if has_geom:
                try:
                    sub_geom = shp_substring(
                        geom, ratios[i], ratios[i + 1], normalized=True
                    )
...
def compute_distance_matrix(G, sensor_node_map, cutoff_m=5000):
    ...
    返回:
        ...
        adj_df: N×N 邻接矩阵 (0/1)
        paths: {(sid_from, sid_to): [node_id, ...]} 最短路节点序列
    """
        ...
        for target_node, dist in lengths.items():
            if target_node in node_to_sid:
                ...
                if target_sid != sid:
                    ...
                    dist_mat[idx, j] = dist
                    paths[(sid, target_sid)] = path_dict[target_node]
                    edge_count += 1
...
def draw_sensor_graph_map(dist_df, adj_df, sensor_node_map, sensors,
                          G_embed, edges_gdf, paths, output_dir):
    """
    在 Folium 地图上绘制传感器图.
    ...
    def node_coord(nid):
        """返回 (lat, lon)."""
        if nid in G_embed:
            return (G_embed.nodes[nid].get("y", 0),
                    G_embed.nodes[nid].get("x", 0))
        return None

    # 辅助: 获取两节点间边的几何坐标序列
    def edge_coords(u, v):
        """返回 u→v 边的 [(lat, lon), ...], 用 geometry 属性或端点直连."""
        edata = G_embed.get_edge_data(u, v)
        if edata:
            # MultiDiGraph 可能多条边, 取第一条
            for key, data in edata.items():
    ...
    for i, sid_from in enumerate(sensor_ids):
        ...
        for j, sid_to in enumerate(sensor_ids):
            ...
            # 获取真实最短路路径
            path_nodes = paths.get((sid_from, sid_to))

            if path_nodes and len(path_nodes) >= 2:
                # 沿路径逐段拼接边几何
                full_coords = []
                for k in range(len(path_nodes) - 1):
                    seg = edge_coords(path_nodes[k], path_nodes[k + 1])
                    if seg:
                        # 避免和上一段末尾重复
                ...
                if len(full_coords) >= 2:
                    folium.PolyLine(
                        full_coords, weight=2, color=color, opacity=0.6,
                        tooltip=(f"{sid_from}→{sid_to} "
                                 f"({dist_m:.0f}m, {len(path_nodes)}节点)"),
                    ).add_to(fg_edges)

                    # 箭头 (用路径最后两个点的方向)
                    lat1, lon1 = full_coords[-2]
                    lat2, lon2 = full_coords[-1]
                ...
                folium.PolyLine(
                    [(lat1, lon1), (lat2, lon2)],
                    weight=1.5, color=color, opacity=0.4,
                    dash_array="5 5",
                    tooltip=(f"{sid_from}→{sid_to} "
                             f"({dist_m:.0f}m, 无路径)"),
                ).add_to(fg_edges)
...
def build_sensor_graph(output_dir="output", cutoff_m=5000):
    ...
    # --- Step 7.3 ---
    dist_df, adj_df, paths = compute_distance_matrix(G_copy, sensor_node_map, cutoff_m)
    ...
    draw_sensor_graph_map(dist_df, adj_df, sensor_node_map, sensors,
                          G_copy, edges_gdf, paths, output_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cutoff", type=float, default=3000,
        help="最短路截断距离 (米, 默认: 3000)",
    )
