...
def phase1_extract_network(bbox_dict, output_dir):
    """
    从 OSM 提取 motorway + motorway_link 网络, 构建关联矩阵.
...
# --- Step 1.1: 从本地 pbf 提取 motorway/motorway_link ---
    log("Step 1.1: 从本地 PBF 文件提取 motorway/motorway_link 网络...")
    import osmium
    ...
    log(f"  使用本地文件: {pbf_path}")

    # --- 第一遍: 收集 bbox 内的 motorway/motorway_link way ---
    class WayCollector(osmium.SimpleHandler):
        ...
        def way(self, w):
            ...
            if hw not in ("motorway", "motorway_link"):
                return
            nodes = []
            in_bbox = False
            for n in w.nodes:
    ...
    # ref 标签覆盖率
    motorway_edges = edge_info_df[edge_info_df["highway"] == "motorway"]
    has_ref = motorway_edges["ref"].apply(lambda x: x != "" and pd.notna(x))
    has_name = motorway_edges["name"].apply(lambda x: x != "" and pd.notna(x))
    log(f"  motorway 边中 ref 标签覆盖率: {has_ref.sum()}/{len(motorway_edges)} "
        f"({100*has_ref.mean():.1f}%)")
    log(f"  motorway 边中 name 标签覆盖率: {has_name.sum()}/{len(motorway_edges)} "
        f"({100*has_name.mean():.1f}%)")
    no_ref_no_name = motorway_edges[~has_ref & ~has_name]
    if len(no_ref_no_name) > 0:
        log(f"  WARNING: {len(no_ref_no_name)} 条 motorway 边既无 ref 也无 name!", "WARN")
...
def phase2_assign_freeways_ref_only(
    ...
    motorway_edges = edge_info_df[edge_info_df["highway"] == "motorway"].copy()
    ...
    for _, row in motorway_edges.iterrows():
        osmid = row["osmid"]
    ...
    log(f"  通过 ref 识别了 {len(link_to_fwy)} 条 motorway 边")
    log(f"  归属到 {len(fwy_groups)} 条高速公路: {sorted(fwy_groups.keys())}")
    log(f"  无 ref 的 motorway 边: {len(no_ref_links)}")
...
def phase4_ramp_matching(
    ...
    # --- Step 4.1: 识别 motorway_link 的 link IDs ---
    log("\nStep 4.1: 识别 motorway_link (匝道) ...")
    ramp_link_ids = set()
    for lid, hw_type in edge_highway_map.items():
        if "motorway_link" in str(hw_type):
            ramp_link_ids.add(lid)
    log(f"  motorway_link 总数: {len(ramp_link_ids)}")
...
def phase6_final_summary(
    ...
    for _, edge in edges_gdf.iterrows():
        if edge.geometry and edge.geometry.geom_type == "LineString":
            ...
            folium.PolyLine(coords, weight=2, color="gray", opacity=0.5).add_to(m)

    # 已匹配传感器 (绿色)
    matched_ids = set()
    if len(all_mapping_df) > 0:
        matched_ids = set(all_mapping_df["sensor_id"].astype(int))
    ...
    for sensor_type, color in colors.items():
        ...
        for _, s in type_df.iterrows():
            sid = int(s["ID"])
            is_matched = sid in matched_ids
            popup = (
                f"ID: {sid}<br>Type: {s['Type']}<br>"
                f"Fwy: {s.get('Fwy','')}<br>Dir: {s.get('Dir','')}<br>"
                f"Name: {s.get('Name','')}<br>"
                f"Matched: {'YES' if is_matched else 'NO'}"
            )
            folium.CircleMarker(
                location=[s["Latitude"], s["Longitude"]],
                radius=4,
                color=color if is_matched else "black",
                fill=True,
                fill_opacity=0.7 if is_matched else 0.3,
                popup=folium.Popup(popup, max_width=300),
                tooltip=f"{sid} ({'✓' if is_matched else '✗'})",
            ).add_to(fg)
        fg.add_to(m)
