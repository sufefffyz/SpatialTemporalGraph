...
用法:
    python review_matching.py output/ d3_motorway.osm

输出:
    output/review_results.csv   - 所有 sensor 的审查结果
...
def load_all_data(output_dir, osm_path):
    ...
    return sensors_df, edge_info_df, mappings, unmatched, way_tags
...
# Step 3: 逐 sensor 审查
# =============================================================================
def review_all_sensors(sensors_df, mappings, unmatched, edge_osm, use_llm=True):
    """对每个已匹配 sensor 进行审查."""
...
# Step 4: 审查未匹配 sensor
# =============================================================================
def review_unmatched(sensors_df, unmatched):
    """汇总未匹配 sensor 的信息."""
...
def run_review(output_dir, osm_path, use_llm=True, pbf_path=None):
    ...
    # Step 1
    sensors_df, edge_info_df, mappings, unmatched, way_tags = \
    ...
    ff_sensors = sensors_df[sensors_df["Type"] == "FF"]
    for _, s in ff_sensors.iterrows():
        sid = int(s["ID"])
        override_rows.append({
            "category": "FF待分配",
            "sensor_id": sid,
