...
# Step 7.4: 输出
# =============================================================================
def save_results(dist_df, adj_df, sensor_node_map, sensors, output_dir):
...
def draw_sensor_graph_map(dist_df, adj_df, sensor_node_map, sensors,
    ...
    for i, sid_from in enumerate(sensor_ids):
        ...
        for j, sid_to in enumerate(sensor_ids):
            ...
            lat1, lon1 = s_from["lat"], s_from["lon"]
            lat2, lon2 = s_to["lat"], s_to["lon"]
    ...
    for stype in ["ML", "OR", "FR"]:
        type_sensors = [s for s in sensors
                        if s["type"] == stype and s["sensor_id"] in sensor_node_map]
        fg = folium.FeatureGroup(
            name=f"{stype} sensors ({len(type_sensors)})", show=True
        )
...
def build_sensor_graph(output_dir="output", cutoff_m=5000):
    ...
    # --- Step 7.4 ---
    save_results(dist_df, adj_df, sensor_node_map, sensors, output_dir)
