...
# OSM relation direction → PeMS Dir 标准化
DIR_NORMALIZE = {
...
def phase2_assign_freeways_ref_only(
    ...
    way_to_rels, output_dir
):
    """
    使用 OSM way ref 标签 + route relation 将主线边分配到高速公路并判定方向.

    优先使用 relation 中的 direction 标签, 回退到坐标推算.
    通过 REF_TO_PEMS_FWY 映射表解决 PeMS 编号与 OSM ref 不一致的问题 (如 SR 51).
    ...
    """
    log("=" * 70)
    log("Phase 2: ref + relation 归属高速公路 + 方向判定")
    log("=" * 70)
    ...
    # --- Step 2.2: 方向判定 (优先用 relation direction, 回退到坐标) ---
    log("\nStep 2.2: 方向判定 (relation direction + 坐标回退)...")

    # 构建 edge_key → {fwy_num → direction} 的 relation 方向查找表
    edge_rel_dir = defaultdict(dict)  # edge_key -> {fwy_num -> "N"/"S"/...}
    for ek, rels in edge_relations.items():
        for rel in rels:
            rel_dir = DIR_NORMALIZE.get(rel["direction"], "")
            if not rel_dir:
                continue
            # 确定该 relation 对应的 PeMS fwy 编号
            fwy_num = ref_to_fwy_num(rel["ref"]) if rel["ref"] else None
            if fwy_num is not None:
                edge_rel_dir[ek][fwy_num] = rel_dir

    # 从 PeMS 元数据获取走向轴 (仅作为坐标回退时使用)
    fwy_dirs = sensors_df.groupby("Fwy")["Dir"].first().to_dict()

    dir_stats = {"from_relation": 0, "from_coordinate": 0, "failed": 0}

    for fwy_num, links in fwy_groups.items():
        for link_info in links:
            ek = link_info["id"]

            # 方法 1: 从 relation direction 获取
            rel_dir = edge_rel_dir.get(ek, {}).get(fwy_num)
            if rel_dir:
                link_info["dir"] = rel_dir
                dir_stats["from_relation"] += 1
                continue
            ...
            if ek not in incidence_df.columns:
                dir_stats["failed"] += 1
                continue
            ...
            try:
                tail_coord = nodes_gdf.loc[int(tail_ids[0])]
                head_coord = nodes_gdf.loc[int(head_ids[0])]
            ...
            if primary_dir in ("N", "S"):
                link_info["dir"] = "S" if tail_coord["y"] > head_coord["y"] else "N"
            elif primary_dir in ("E", "W"):
                link_info["dir"] = "W" if tail_coord["x"] > head_coord["x"] else "E"

            if link_info["dir"] is not None:
                dir_stats["from_coordinate"] += 1
            else:
                dir_stats["failed"] += 1

    log(f"  方向判定结果: relation={dir_stats['from_relation']}, "
        f"坐标推算={dir_stats['from_coordinate']}, 失败={dir_stats['failed']}")
