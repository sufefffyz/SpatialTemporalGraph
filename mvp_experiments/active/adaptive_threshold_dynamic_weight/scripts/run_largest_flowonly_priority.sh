...
GRAPH="${2:-}"
GPU="${3:-0}"
RUN_TAG="${4:-largest_flowonly_priority_$(date '+%Y%m%d_%H%M%S')}"
...
if [[ "${ADDAPT}" == "1" && "${MODEL}" != "gwnet" ]]; then
  echo "Adaptive adjacency variants are only valid for GWNet, got MODEL=${MODEL} GRAPH=${GRAPH}" >&2
  exit 2
fi

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
BASICTS_DIR="${REPO_ROOT}/BasicTS"
LOG_ROOT="${BASICTS_DIR}/logs/adaptive_threshold_dynamic_weight/largest_flowonly/${RUN_TAG}/${MODEL}"
mkdir -p "${LOG_ROOT}"

case "${MODEL}:${GRAPH_KIND}" in
  gwnet:original)
    CFG="baselines/GWNet/SD_largest_flowonly.py"
    MODEL_NAME="GraphWaveNet"
    ;;
  gwnet:topk_prior)
    CFG="baselines/GWNet/SD_topk_prior_largest_flowonly.py"
    MODEL_NAME="GraphWaveNet"
    ;;
  gwnet:dynamic_threshold)
    CFG="baselines/GWNet/SD_dynamic_threshold_largest_flowonly.py"
    MODEL_NAME="DynamicThresholdGraphWaveNet"
    ;;
  dcrnn:original)
    CFG="baselines/DCRNN/SD_largest_flowonly.py"
    MODEL_NAME="DCRNN"
    ;;
  dcrnn:topk_prior)
    CFG="baselines/DCRNN/SD_topk_prior_largest_flowonly.py"
    MODEL_NAME="DCRNN"
    ;;
  dcrnn:dynamic_threshold)
    CFG="baselines/DCRNN/SD_dynamic_threshold_largest_flowonly.py"
    MODEL_NAME="DynamicThresholdDCRNN"
    ;;
  stgcn:original)
    CFG="baselines/STGCN/SD_largest_flowonly.py"
    MODEL_NAME="STGCNChebGraphConv"
    ;;
  stgcn:topk_prior)
    CFG="baselines/STGCN/SD_topk_prior_largest_flowonly.py"
    MODEL_NAME="STGCNChebGraphConv"
    ;;
  stgcn:dynamic_threshold)
    CFG="baselines/STGCN/SD_dynamic_threshold_largest_flowonly.py"
...
{
  echo "model,graph,dataset,candidate_adj,gpu,config,run_tag,protocol,log"
  if [[ "${DYNAMIC}" == "1" && -n "${CANDIDATE_DATASET}" ]]; then
    echo "${MODEL},${GRAPH},${DATASET_NAME},datasets/${CANDIDATE_DATASET}/adj_mx.pkl,${GPU},${CFG},${RUN_TAG},largest_flowonly,${log_file}"
  else
    echo "${MODEL},${GRAPH},${DATASET_NAME},,${GPU},${CFG},${RUN_TAG},largest_flowonly,${log_file}"
  fi
...
export WANDB_RUN_GROUP="sd_largest_flowonly_${MODEL}_${GRAPH_KIND}_${CANDIDATE_TAG}_${ADDAPT_TAG}"
export WANDB_TAGS="adaptive-threshold,sd,largest-flowonly,flow-only,${MODEL},${GRAPH_KIND},${GRAPH_TAG},${CANDIDATE_TAG},${ADDAPT_TAG}"
export WANDB_NAME="${MODEL_NAME}_${DATASET_NAME}_${GRAPH}_${GRAPH_KIND}_${RUN_TAG}_flowonly"
...
echo "[$(date '+%F %T')] Starting ${MODEL} ${GRAPH} (${GRAPH_KIND}, largest-flowonly) on GPU ${GPU}" | tee -a "${log_file}"
/home/yuzhang_fei/miniconda3/envs/STGraph/bin/python experiments/train.py -c "${CFG}" -g "${GPU}" 2>&1 | tee -a "${log_file}"
echo "[$(date '+%F %T')] Finished ${MODEL} ${GRAPH} (${GRAPH_KIND}, largest-flowonly)" | tee -a "${log_file}"
