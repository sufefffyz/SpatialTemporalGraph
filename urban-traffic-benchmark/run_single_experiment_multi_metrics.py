...
def configure_runtime_warnings() -> None:
    # DGL emits this deprecation warning very frequently and it can dominate log I/O.
    warnings.filterwarnings(
        'ignore',
        message=r'.*torch\.cuda\.amp\.autocast_mode\._cast\(value, dtype\).*',
...
def get_args(add_name: bool = True):
    ...
    parser.add_argument('--report_metrics', nargs='*', type=str, default=['auto'],
                        help='Metrics to report on val/test. Supported: MAE MSE RMSE MAPE. '
                            'Use auto to pick a default set based on --metric.')
    parser.add_argument('--mape_eps', type=float, default=1e-6,
                        help='Small epsilon for MAPE denominator to avoid division by values close to zero.')
    parser.add_argument('--best_run_predictions_dir', type=str, default=None,
                        help='Directory where best-run prediction files will be saved '
                            '(best_run_XX_predictions.npz). '
                            'If not set, defaults to the current experiment directory.')
    parser.add_argument('--best_run_artifacts_dir', type=str, default=None,
                        help='Deprecated alias of best_run_predictions_dir. '
                            'Only affects predictions path for backward compatibility.')
...
def compute_metric(preds, targets, targets_nan_mask, dataset, loss_fn, metric, apply_transform_to_preds=True):
    # Targets are already in metric space; keep a reference to avoid extra copy.
    ...
    if len(preds) < dataset.eval_max_num_timestamps_per_step:
        ...
        # Keep transformed predictions on CPU for downstream metrics/artifacts.
        if apply_transform_to_preds:
            preds_transformed = dataset.transform_preds_for_metrics(preds.to(dataset.device)).cpu()
        else:
            preds_transformed = preds.clone()

        loss_sum = 0
        loss_count = 0
        chunk_size = dataset.eval_max_num_timestamps_per_step
        for start_idx in range(0, len(preds), chunk_size):
            end_idx = min(start_idx + chunk_size, len(preds))

            cur_preds = preds[start_idx:end_idx].to(dataset.device)
            if apply_transform_to_preds:
                cur_preds = dataset.transform_preds_for_metrics(cur_preds)

            cur_targets = targets[start_idx:end_idx].to(dataset.device)
            cur_targets_nan_mask = targets_nan_mask[start_idx:end_idx].to(dataset.device)
...
def resolve_report_metrics(primary_metric, report_metrics):
    primary_metric = primary_metric.upper()
    normalized = [m.upper() for m in report_metrics]

    if 'AUTO' in normalized:
        if primary_metric == 'MAE':
            normalized = ['MAE', 'RMSE', 'MAPE']
        elif primary_metric == 'RMSE':
            normalized = ['RMSE', 'MAE', 'MAPE']
        else:
            normalized = [primary_metric]

    supported = {'MAE', 'MSE', 'RMSE', 'MAPE'}
    ordered = []
    for metric_name in [primary_metric] + normalized:
        if metric_name not in supported:
            raise ValueError(f'Unsupported metric in report_metrics: {metric_name}. Supported: {sorted(supported)}')
...
def compute_metrics_from_transformed(preds_transformed, targets_transformed, targets_nan_mask, metric_names, mape_eps):
    preds = preds_transformed.float()
    targets = targets_transformed.float()
    valid_mask = ~targets_nan_mask.bool()
    ...
    if 'MAE' in metric_names:
        metrics['MAE'] = float(torch.abs(err)[valid_mask].mean().item())

    if 'MSE' in metric_names or 'RMSE' in metric_names:
        mse_val = float((err.pow(2))[valid_mask].mean().item())
        metrics['MSE'] = mse_val
        if 'RMSE' in metric_names:
            metrics['RMSE'] = float(np.sqrt(mse_val))

    if 'MAPE' in metric_names:
        # By definition here, points with zero ground truth are excluded from MAPE.
        non_zero_mask = valid_mask & (targets != 0)
        if int(non_zero_mask.sum().item()) == 0:
            metrics['MAPE'] = float('nan')
        else:
            metrics['MAPE'] = float((torch.abs(err[non_zero_mask] / targets[non_zero_mask]).mean().item()) * 100.0)

    return {name: metrics[name] for name in metric_names}


def save_best_run_artifacts(logger, run_id, step, epoch, metrics, do_not_evaluate_on_test,
                            best_run_predictions_dir=None):
    predictions_dir = logger.save_dir if best_run_predictions_dir is None else best_run_predictions_dir
    os.makedirs(predictions_dir, exist_ok=True)

    pred_path = os.path.join(predictions_dir, f'best_run_{run_id:02d}_predictions.npz')
    metrics_path = os.path.join(logger.save_dir, f'best_run_{run_id:02d}_metrics.yaml')

    arrays = {}
    if VAL_PREDICTIONS is not None:
        arrays['val_predictions'] = VAL_PREDICTIONS.detach().cpu().numpy()
    if VAL_TARGETS is not None:
        arrays['val_targets'] = VAL_TARGETS.detach().cpu().numpy()
    if VAL_TARGETS_NAN_MASK is not None:
        arrays['val_targets_nan_mask'] = VAL_TARGETS_NAN_MASK.detach().cpu().numpy()

    if not do_not_evaluate_on_test:
        if TEST_PREDICTIONS is not None:
            arrays['test_predictions'] = TEST_PREDICTIONS.detach().cpu().numpy()
        if TEST_TARGETS is not None:
            arrays['test_targets'] = TEST_TARGETS.detach().cpu().numpy()
        if TEST_TARGETS_NAN_MASK is not None:
            arrays['test_targets_nan_mask'] = TEST_TARGETS_NAN_MASK.detach().cpu().numpy()
    ...
    metrics_to_save.update({k: float(v) for k, v in metrics.items()})

    with open(metrics_path, 'w') as file:
        yaml.safe_dump(metrics_to_save, file, sort_keys=False)


def update_metrics_yaml_with_all_metrics(logger, metric_names, do_not_evaluate_on_test):
    metrics_yaml_path = os.path.join(logger.save_dir, 'metrics.yaml')
    if not os.path.exists(metrics_yaml_path):
        return

    with open(metrics_yaml_path, 'r') as file:
        base_metrics = yaml.safe_load(file) or {}

    per_run_metrics = []
    for run_id in range(1, len(logger.val_metrics) + 1):
        run_metrics_path = os.path.join(logger.save_dir, f'best_run_{run_id:02d}_metrics.yaml')
        if not os.path.exists(run_metrics_path):
            continue

        with open(run_metrics_path, 'r') as file:
            run_metrics = yaml.safe_load(file) or {}
        per_run_metrics.append(run_metrics)

    if not per_run_metrics:
        return

    splits = ['val'] if do_not_evaluate_on_test else ['val', 'test']
    for split in splits:
        for metric_name in metric_names:
            key = f'{split} {metric_name}'
            values = [float(metrics[key]) for metrics in per_run_metrics if key in metrics]
            if not values:
                continue

            values_np = np.array(values, dtype=np.float64)
            base_metrics[f'{key} mean'] = float(values_np.mean().item())
            base_metrics[f'{key} std'] = float(values_np.std(ddof=1).item()) if len(values) > 1 else math.nan
            base_metrics[f'{key} values'] = values

    with open(metrics_yaml_path, 'w') as file:
        yaml.safe_dump(base_metrics, file, sort_keys=False)
...
def evaluate_on_val_or_test(model, dataset, split, timestamps_loader, loss_fn, metric, amp=True):
    ...
    metric_val, preds_transformed, targets_transformed = compute_metric(preds=preds, targets=targets, targets_nan_mask=targets_nan_mask, dataset=dataset,
...
def evaluate(model, dataset, val_timestamps_loader, test_timestamps_loader, loss_fn, metric, amp=True,
             do_not_evaluate_on_test=False, report_metric_names=None, mape_eps=1e-6):
    report_metric_names = resolve_report_metrics(metric, report_metric_names or ['auto'])

    primary_metric_name = metric.upper()
    metrics = {}
    detailed_metrics = {}
    _ = evaluate_on_val_or_test(model=model, dataset=dataset, split='val',
                                timestamps_loader=val_timestamps_loader, loss_fn=loss_fn,
                                metric=metric, amp=amp)
    val_metrics = compute_metrics_from_transformed(
        preds_transformed=VAL_PREDICTIONS,
        targets_transformed=VAL_TARGETS,
        targets_nan_mask=VAL_TARGETS_NAN_MASK,
        metric_names=report_metric_names,
        mape_eps=mape_eps,
    )
    val_metrics[primary_metric_name] = val_metrics.get(primary_metric_name, val_metrics.get('MAE'))
    metrics[f'val {primary_metric_name}'] = val_metrics[primary_metric_name]
    for metric_name, metric_value in val_metrics.items():
        detailed_metrics[f'val {metric_name}'] = metric_value

    if not do_not_evaluate_on_test:
        _ = evaluate_on_val_or_test(model=model, dataset=dataset, split='test',
                                    timestamps_loader=test_timestamps_loader, loss_fn=loss_fn,
                                    metric=metric, amp=amp)
        test_metrics = compute_metrics_from_transformed(
            preds_transformed=TEST_PREDICTIONS,
            targets_transformed=TEST_TARGETS,
            targets_nan_mask=TEST_TARGETS_NAN_MASK,
            metric_names=report_metric_names,
            mape_eps=mape_eps,
        )
        test_metrics[primary_metric_name] = test_metrics.get(primary_metric_name, test_metrics.get('MAE'))
        metrics[f'test {primary_metric_name}'] = test_metrics[primary_metric_name]
        for metric_name, metric_value in test_metrics.items():
            detailed_metrics[f'test {metric_name}'] = metric_value
...
def train(model, dataset, loss_fn, metric, logger: Logger, num_epochs, num_accumulation_steps, eval_every, lr,
          ...
          do_not_evaluate_on_test=False, nirvana=False, report_metric_names=None, mape_eps=1e-6,
          best_run_predictions_dir=None):

    resolved_report_metric_names = resolve_report_metrics(metric, report_metric_names or ['auto'])
    ...
    tqdm_kwargs = {'total': num_steps, 'desc': f'Run {run_id}'}
    if not sys.stderr.isatty():
        # In redirected logs (nohup/file), reduce progress redraw overhead.
        tqdm_kwargs.update(mininterval=5.0, miniters=20, dynamic_ncols=False)
    ...
    with tqdm(**tqdm_kwargs) as progress_bar:
        ...
        for step in range(starting_step_idx + 1, num_steps + 1):
            ...
            progress_bar.update()
            should_refresh_postfix = sys.stderr.isatty() or step <= 20 or step % 20 == 0
            if should_refresh_postfix:
            ...
            if (
                ...
                model.eval()
                metrics, detailed_metrics = evaluate(model=model, dataset=dataset, val_timestamps_loader=val_timestamps_loader,
                                ...
                                report_metric_names=resolved_report_metric_names, mape_eps=mape_eps)

                best_val_so_far = logger.val_metrics[-1]
                cur_val = metrics[f'val {metric.upper()}']
                if best_val_so_far is None or cur_val < best_val_so_far:
                    save_best_run_artifacts(
                        ...
                        best_run_predictions_dir=best_run_predictions_dir,
                    )
    ...
    logger.finish_run()
    update_metrics_yaml_with_all_metrics(
        logger=logger,
        metric_names=resolved_report_metric_names,
        do_not_evaluate_on_test=do_not_evaluate_on_test,
    )
...
def main():
    ...
    for run in range(state_handler.num_runs_completed + 1, args.num_runs + 1):
        ...
        train(model=model, dataset=dataset, loss_fn=loss_fn, metric=args.metric, logger=logger,
              ...
              report_metric_names=args.report_metrics, mape_eps=args.mape_eps,
              best_run_predictions_dir=args.best_run_predictions_dir or args.best_run_artifacts_dir,
              )
