...
class BaseEngine():
    def __init__(self, device, model, dataloader, scaler, sampler, loss_fn, lrate, optimizer, \
                 scheduler, clip_grad_value, max_epochs, patience, log_dir, logger, 
                 args):
        super().__init__()
        self._device = device
        self.model = model
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_c{}.pt'.format(self._args.core)
    ...
    def train_batch(self):
        ...
        for X, label in tqdm(self._dataloader['train_loader'].get_iterator()):
            ...
            X, label = self._to_device(self._to_tensor([X, label[:, :self._args.horizon]]))
